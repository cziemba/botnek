...
- **[M] CI.** GitHub Actions running `pnpm run build`, `pnpm run lint`, and `pnpm test` (unit tests only — exclude `*.integration.test.ts` so live-YouTube flakiness doesn't break PRs). Gate on the existing `mainline` → `main` merge.
